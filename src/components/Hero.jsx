...
export default function Hero() {
  ...
  return (
    <section className="relative w-full min-h-screen bg-[#FAF8F4] flex flex-col overflow-hidden pt-[80px]" style={ROBOTO}>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          style={{ color: '#0A0A0A', opacity: 0.03, ...ROBOTO, lineHeight: 1 }}
          initial={{ opacity: 0 }}
      ...
      <div className="flex-1 flex flex-col lg:flex-row max-w-[1400px] mx-auto w-full px-6 sm:px-10 xl:px-16">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-5">
                ...
                <span className="text-[11px] font-bold tracking-[0.16em] text-[#0877BA] uppercase" style={ROBOTO}>
                  {s.tag}
                </span>
              </div>
              ...
              {/* heading */}
              <h1
                className="font-black text-[#0A0A0A] leading-[1.07] tracking-tight mb-6"
                style={{ fontSize: 'clamp(40px, 6vw, 72px)', ...ROBOTO }}
              >
                {s.titleTop} <br />
                {s.titleBig} <br />
                {s.titleSub}
              ...
              <p className="text-gray-500 text-[15px] leading-[1.75] max-w-[500px] mb-6" style={ROBOTO}>
                {s.desc}
              </p>

              {/* cities */}
              <div className="flex items-center gap-2 mb-8">
                <FaMapMarkerAlt size={12} color="#0877BA" />
                <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider" style={ROBOTO}>
                  {s.cities}
                </span>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4 mb-10">
                <a
                  href="#programs"
                  className="inline-flex items-center gap-2 bg-[#0877BA] text-white font-bold text-[12px] uppercase tracking-[0.14em] px-8 py-4 hover:bg-[#0A0A0A] transition-colors duration-200 group"
                  style={ROBOTO}
                >
                  {s.cta}
                ...
                <a
                  ...
                  style={ROBOTO}
                >
                  <FaPlay size={10} color="#0A0A0A" />
                  Watch Stories
                </a>
              </div>

              {/* stat badge */}
              <div className="inline-flex items-center gap-3 bg-white border border-[#0877BA]/15 shadow-sm px-5 py-3">
                <span className="text-[24px] font-black text-[#0877BA]" style={ROBOTO}>{s.stat.num}</span>
                <span className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider" style={ROBOTO}>{s.stat.label}</span>
              </div>
        ...
        <div className="relative lg:w-[45%] flex items-end justify-center min-h-[380px] overflow-hidden">
          ...
          <AnimatePresence mode="wait">
            <motion.div
              key={`card-${current}`}
              className="absolute top-8 left-4 z-20 bg-white/90 backdrop-blur-sm border border-[#0877BA]/15 shadow-lg px-4 py-3"
              initial={{ opacity: 0, x: -12 }}
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0 }}
              transition={{ delay: 0.3, duration: 0.35 }}
            >
              <div className="text-[9px] font-bold text-gray-400 tracking-widest uppercase mb-0.5" style={ROBOTO}>Featured Program</div>
              <div className="text-[13px] font-black text-[#0A0A0A]" style={ROBOTO}>
                {current === 0 ? 'Tokyo University' : current === 1 ? 'Kyoto Institute' : 'Waseda University'}
              </div>
              <div className="text-[10px] text-[#F6C21F] mt-0.5" style={ROBOTO}>★★★★★ Top Ranked</div>
            </motion.div>
          </AnimatePresence>
      ...
      <div className="border-t border-black/8 bg-white/60 backdrop-blur-sm">
        <div className="max-w-[1400px] mx-auto px-6 sm:px-10 xl:px-16 py-4 flex items-center justify-between gap-6">
          ...
          <div className="flex items-baseline gap-1.5">
            <span className="text-[28px] font-black text-[#0877BA] leading-none" style={ROBOTO}>0{current + 1}</span>
            <span className="text-gray-300 text-lg" style={ROBOTO}>/</span>
            <span className="text-[14px] text-gray-400 font-medium" style={ROBOTO}>0{SLIDES.length}</span>
          </div>
          ...
          <div className="flex gap-2 flex-1 max-w-[240px]">
            {SLIDES.map((_, i) => (
              ...
              >
                <div
                  className="h-full bg-[#0877BA] rounded-full transition-none"
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={next}
              className="w-9 h-9 bg-[#0877BA] text-white flex items-center justify-center hover:bg-[#0A0A0A] transition-colors duration-200"
            >
